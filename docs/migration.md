...
| Field CAD scene scale | Retain metres-per-viewport-unit, its preset range, and the divide-before-narrowing conversion that makes atomic and astronomical scenes reachable by one camera. Replace its world-document field and `SetSceneScale` world command with client-owned default-view state that never enters experiment intent, undo, workload identity, or the MCP surface. | [K14](tasks/kagami/choose-scene-scale.md) and ADR 0022 |
