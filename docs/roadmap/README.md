...
- **S-IDENTITY:** explicit `FormationId`, formation-assigned `NodeId`, cluster
  ...
  JSON/CBOR fixtures. Membership/formation-v1 and the shared
  [run descriptor with single-node owner allocation](../run-descriptor-v1.md)
  are implemented. Reconcile the remaining generic runtime resource projections
  and public RunReference routes; distributed/reset epoch allocation is not closed
  by single-node evidence.
...
- Complete remaining [X-PLUGIN conformance evidence](../tasks/define-and-implement-plugin-contract.md#slice-0--finish-the-contract)
  ...
  resolver/dormancy tests now exist; local inventory and migration evidence remains.
