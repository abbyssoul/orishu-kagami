#!/usr/bin/env python3
"""Bounded 3–5-host / 1-or-4-workers-per-host Unix or off-host HTTPS diagnostic.
...
def read_inventory(path):
    ...
    require(3 <= len(value['nodes']) <= 5, 'capacity requires three to five physical hosts')
    return value


def review_error_details(details, errors, clients):
    """Finite categories/timing only; no raw server strings or inferred timeouts."""
    kinds = {'serialization', 'connection', 'transport_empty_body', 'transport_body_decode',
             'transport_body_read', 'transport_other', 'authentication', 'authorization',
             'not_found', 'unexpected_response', 'api', 'invalid_state'}
    require(isinstance(details, dict) and set(details) == {'counts', 'first'}, 'error detail fields')
    counts = details['counts']
    require(isinstance(counts, list) and len(counts) <= clients and errors <= clients, 'error detail cap')
    seen, total = set(), 0
    for row in counts:
        require(isinstance(row, dict) and set(row) == {'kind', 'status', 'count'}
                and isinstance(row['kind'], str) and row['kind'] in kinds, 'error category')
        status = row['status']
        require((type(status) is int and 0 <= status <= 65535) if row['kind'] == 'api' else
                (type(status) is int and 100 <= status <= 599) if row['kind'] == 'transport_empty_body' else
                status is None, 'error status')
        key = (row['kind'], status)
        require(key not in seen and type(row['count']) is int and 1 <= row['count'] <= clients,
                'duplicate/invalid error count')
        seen.add(key)
        total += row['count']
    require(total == errors, 'error counts disagree with total')
    first = details['first']
    if not errors:
        require(first is None, 'unexpected first error')
        return
    require(isinstance(first, dict) and set(first) == {'kind', 'status', 'request_started_us',
            'completed_us', 'duration_us', 'after_window'}, 'first error fields')
    require(isinstance(first['kind'], str) and (first['status'] is None or type(first['status']) is int)
            and (first['kind'], first['status']) in seen, 'first error category')
    require(all(type(first[k]) is int and 0 <= first[k] <= 60_000_000
                for k in ('request_started_us', 'completed_us', 'duration_us')),
            'first error timing bound')
    require(first['completed_us'] - first['request_started_us'] == first['duration_us']
            and type(first['after_window']) is bool, 'first error duration')
    require(first['completed_us'] == 10_000_000 or
            first['after_window'] == (first['completed_us'] > 10_000_000), 'first error window')
...
def review_load(value, expected, *, network=False):
    ...
    require(value['schema_version'] == expected.get('schema_version', 2 if network else 1)
            and value['kind'] == ('pi-network-capacity-load' if network else 'pi-capacity-load')
            and value['global_workers'] == expected.get('global_workers', 16) and value['seconds'] == 10, 'capacity load version/window')
    ...
    require(isinstance(value['workers'], list) and len(value['workers']) == len(expected.get('targets', expected.get('nodes', [None] * 4))), 'capacity rows')
    rows = []
    detailed = value['schema_version'] == (4 if network else 3)
    for slot, row in enumerate(value['workers']):
        fields = {'role', 'latency', 'scheduling_delay', 'scheduled_arrivals', 'skipped_arrivals',
                  'tail_requests', 'transport_errors', 'invalid_responses', 'capped'}
        require(set(row) == fields | ({'client_error_details'} if detailed else set())
        ...
        if detailed:
            review_error_details(row['client_error_details'], row['transport_errors'], expected['clients_per_worker'])
...
def execute_curve(cell, baseline, *, short_comparison=False, error_diagnostic=False):
    """Select windows only; the same measured cell retains every safety gate."""
    if error_diagnostic:
        for clients in (32, 64):
            cell(None, clients)
        return
    if short_comparison:
        for _ in range(2):
...
def run(inventory, output, digest, baseline, policy, max_elapsed_seconds=600, *, network=None,
        short_comparison=False, workers_per_host=4, executor_threads=None, placement=False,
        error_diagnostic=False):
    from pi_lab_session import experiment_options
    hosts = len(inventory['nodes'])
    options = experiment_options({'hosts': hosts, 'workers_per_host': workers_per_host,
                                  'executor_threads': executor_threads, 'placement': placement})
    local, total = workers_per_host, hosts * workers_per_host
    ...
    require(type(error_diagnostic) is bool and (not error_diagnostic or
            (network is not None and not short_comparison)), 'error diagnostic requires exclusive network profile')
    require(type(max_elapsed_seconds) is int and 180 <= max_elapsed_seconds <= 600, 'whole experiment bound')
    require(isinstance(inventory, dict) and 3 <= hosts <= 5, 'physical host count')
    ...
    result = {'schema_version': 1, 'kind': 'pi-capacity-diagnostic', 'run_id': run_id,
              ...
              'policy': policy, 'experiment': options, 'cleanup': []}
    result['transport'] = 'authenticated-https-off-host' if network else 'colocated-unix'
    result['curve'] = ('two-profile-error-diagnostic' if error_diagnostic else
                       'three-profiles-twice' if short_comparison else 'adaptive-capacity')
    ...
    def rpc(role, operation, **args):
        return sessions[role // local].worker(role % local, operation, **args)
    ...
    def all_views():
        def host_views(host):
            return [sessions[host].worker(slot, 'info') for slot in range(local)]
        with ThreadPoolExecutor(max_workers=hosts) as pool:
            return [view for group in pool.map(host_views, range(hosts)) for view in group]

    def verify():
        views = all_views()
        lists = [rpc(role, 'members') for role in range(total)]
        require(all(isinstance(row, list) and len(row) <= total for row in lists), 'capacity membership count')
    ...
    def cell(rate, clients):
        ...
        for host, session in enumerate(sessions):
            ...
            args = {'cell': number, 'first_role': host * local, 'rate_per_worker': rate,
                    'clients_per_worker': clients, 'formation': formation,
                    'nodes': assigned[host * local:host * local + local], 'probe_sha256': digest,
                    'global_workers': total}
            ...
            expected = {'schema_version': 2, 'global_workers': total, 'first_role': host * local, 'rate_per_worker': rate,
                        'clients_per_worker': clients, 'targets': [
                            {'socket': str(Path(session.node['root']) / 'runs' / f'{run_id}-{slot}' / 'api.sock'),
                             'formation': formation, 'node': assigned[host * local + slot]} for slot in range(local)]}
            if network:
        ...
        errors = []
        with ThreadPoolExecutor(max_workers=hosts) as pool:
            futures = {pool.submit(measure, host): host for host in range(hosts)}
            for future in as_completed(futures):
                ...
                try:
                    ...
                    require(len(report['resources']) == (local + (1 if network else 2)), 'process resource count')
    ...
    try:
        ...
        event(f'prepared-{hosts}-hosts')
        for role in range(total):
            rpc(role, 'start')
        initial = all_views()
        require(len({v['formationId'] for v in initial}) == total, 'standalone identities')
        pins = [rpc(role, 'members')[0]['certFingerprint'] for role in range(total)]
        require(len(set(pins)) == total, 'certificate uniqueness')
        formation, assigned = initial[0]['formationId'], [initial[0]['sourceNodeId']]
        for role in range(1, total):
            # One chain crosses every physical host; local groups remain
            # members of the same formation, not independent clusters.
            issuer = role - 1
        ...
        event(f'formed-{total}-workers')
        execute_curve(cell, baseline, short_comparison=short_comparison, error_diagnostic=error_diagnostic)
        ...
        result['cleanup_clean'] = len(sessions) == hosts and all(row['clean'] for row in result['cleanup'])
...
def main():
    ...
    parser.add_argument('--error-diagnostic', action='store_true',
                        help='Network only: one unpaced 32/64-client pair, stopping on the first error; exclusive with short comparison.')
    ...
    parser.add_argument('--workers-per-host', type=int, choices=(1, 4), default=4)
    parser.add_argument('--executor-threads', type=int, choices=(1, 2, 4),
                        help='Disposable worker Tokio multi-thread pool size; omission retains default.')
    parser.add_argument('--placement', action='store_true',
                        help='Enforce inventory interface for peers and network client replies.')
    ...
    result = run(read_inventory(args.inventory), args.output, args.probe_sha256, args.baseline_per_worker,
                 ...
                 workers_per_host=args.workers_per_host, executor_threads=args.executor_threads,
                 placement=args.placement, error_diagnostic=args.error_diagnostic)
