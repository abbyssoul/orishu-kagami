...
1. (Optional) For testing docker build, install [Docker] or [Podman] (recommended).
...
More often then note, unit tests are insufficient and complementary techniques should be used:

1. Property-based tests (aka. fuzzing).
...
Trust is not given, and nodes cannot assume that network messages are
well-formed, even when they come from authenticated clients or known peers.
For that reason, all network-facing code is expected to be thoroughly
fuzz-tested. The baseline reliability standard is: "no single message should
be able to bring a node down or disrupt the cluster".
...
### Variable naming
Variable names are always `nouns`!
> A noun is a part of speech that names a person, place, thing, or idea, acting as a foundational building block for sentences. Nouns function primarily as subjects or objects in a sentence and can be concrete (physical objects) or abstract (concepts).
...
### Function naming
> A verb is a part of speech that describes an action (e.g., run, jump), an occurrence (e.g., happen, become), or a state of being (e.g., be, exist).

Use verbs. Stay concise. Use the function doc comment to describe what the function does,
...
As such all project artifacts must be published using [GitHub workflows](.github/workflows/) automatons.
...
Should you need to build and publish a multi-arch image manually:

```sh
docker buildx build \
  --target orishu-worker \
  --platform linux/amd64,linux/arm64 \
  --tag ghcr.io/abbyssoul/orishu-worker:$(git rev-parse --short HEAD) \
  --provenance=true \
  --sbom=true \
  --push .
```
...
## Debian Packages
The workspace ships `cargo-deb` metadata for three separate Debian packages:

- `orishu-worker`
- `orishuctl`
- `orishu-monitor`

Build them locally from the workspace root:

```sh
cargo deb --locked -p orishu-worker
cargo deb --locked -p orishuctl
cargo deb --locked -p orishu-monitor
```

Artifacts are written to `target/debian/`.

The worker package includes:

- `/usr/bin/orishu-worker`
- `/etc/orishu/orishu-worker.conf`
- the packaged `orishu-worker.service`
- the TLS credential drop-in example under `/usr/share/doc/orishu-worker/examples/`

The packaged worker config stays local-socket-first by default. Do not reintroduce a plaintext
public `listen.clients` default in packaging; TCP listeners are an operator opt-in and should be
paired with TLS.

## Debian Release Workflow
GitHub Actions builds Debian packages in `.github/workflows/publish-debs.yml`.

- `release/*` pushes build `amd64` and `arm64` packages and upload them as workflow artifacts
- `v*` tags build the same packages and attach them to the matching GitHub Release
